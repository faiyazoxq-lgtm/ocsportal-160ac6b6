...
export function BossShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-background pt-10">
      <div className="fixed inset-x-0 top-0 z-50 flex h-10 items-center gap-2 border-b border-indigo-500/30 bg-indigo-600 px-5 text-[13px] font-bold uppercase tracking-wider text-white shadow-sm">
      ...
      <div className="flex flex-1">
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-4">
          <div className="mb-1.5 px-2 text-[10px] font-semibold uppercase tracking-wider text-sidebar-foreground/50">Boss</div>
          {BOSS_NAV.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`mb-0.5 flex items-center gap-2.5 rounded-sm px-3 py-2 text-sm ${
                  active
          ...
          <div className="mb-1.5 mt-5 px-2 text-[10px] font-semibold uppercase tracking-wider text-sidebar-foreground/50">Operations</div>
          {OPS_NAV.map((item) => {
            const Icon = item.icon;
        ...
        <div className="border-t border-sidebar-border p-3 text-xs text-sidebar-foreground/70">
          <div className="mb-2 flex items-center gap-2">
            <UserAvatar url={profile?.avatar_url} name={profile?.full_name || profile?.email} size={32} />
            <div className="min-w-0 flex-1 truncate">{profile?.full_name || profile?.email}</div>
          </div>
          <Link
            to="/account"
            className="mb-1 flex w-full items-center gap-2 rounded-sm px-2 py-2 text-sm hover:bg-sidebar-accent/40"
          >
            <KeyRound className="h-4 w-4" /> Account & password
          </Link>
          <button
            onClick={() => void signOut()}
            className="flex w-full items-center gap-2 rounded-sm px-2 py-2 text-sm hover:bg-sidebar-accent/40"
          >

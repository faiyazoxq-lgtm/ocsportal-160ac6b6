...
export function EngineerShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-background pt-10">
      <div className="fixed inset-x-0 top-0 z-50 flex h-10 items-center gap-2 border-b border-emerald-500/30 bg-emerald-600 px-4 text-[13px] font-bold uppercase tracking-wider text-white shadow-sm">
        <Wrench className="h-4 w-4" />
        Engineer Console
      </div>
      <header className="sticky top-10 z-20 flex h-16 items-center justify-between border-b border-border bg-card px-4">
        <Logo />
        ...
        {NAV.map((item) => {
          ...
          if (item.enabled) {
            return (
              <Link
                ...
                className={`flex min-h-[56px] flex-col items-center justify-center gap-1 py-2 text-xs font-medium transition-colors active:bg-accent/40 ${
                  active
                    ? "text-primary"
          ...
          return (
            <button
              ...
              disabled
              aria-current={active ? "page" : undefined}
              className={`flex min-h-[56px] flex-col items-center justify-center gap-1 py-2 text-xs font-medium transition-colors disabled:cursor-not-allowed ${
                active
                  ? "text-primary"
                  : "text-muted-foreground hover:text-foreground"
              }`}

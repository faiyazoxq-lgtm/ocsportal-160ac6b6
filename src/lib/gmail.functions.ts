...
/**
 * Build the intake_records row(s) for a Gmail message:
 *  - downloads attachments + email body
 *  - asks Gemini vision to extract ONE OR MULTIPLE work orders
 *  - creates one intake_records row per detected work order, pre-populated
 *    with extracted_fields_json + suggested_categorization_json so the
 *    dispatcher/boss can verify time + quote and convert into the
 *    awaiting-dispatch queue.
 *
 * Falls back to a single empty intake row if AI extraction yields nothing.
 * Returns the inserted intake row IDs in detection order.
 */
async function createIntakeFromGmail(args: {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Dispatcher/boss must always verify time + quotation before approval —
    // so every AI-extracted row lands in needs_review (never auto-parsed).

...
- tenant_name: the SINGLE primary tenant / resident / occupier / on-site contact. If multiple tenants/site contacts are listed, put the FIRST (or most-clearly-primary) one here and put each additional tenant in additional_contacts with role "tenant" — pair each name with its matching phone/email by position when the source lists them in the same order. NEVER concatenate multiple names with commas in this field.
- tenant_phone: the phone number for the SINGLE primary tenant above. Pair it with tenant_name by position when the source lists names and numbers in the same order. Additional tenant numbers must go into additional_contacts rows alongside their owner's name, NOT comma-joined here. Do NOT use office/branch phone as tenant_phone.
- tenant_email: the email for the SINGLE primary tenant above. Additional tenant emails go in additional_contacts. Do NOT use office/branch email as tenant_email.
